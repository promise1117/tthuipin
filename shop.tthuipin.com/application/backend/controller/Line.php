...
//        $this->check = Validate('Category');
    ...
    public function lineList()
    {
        ...
        $data = Db('line')->where($map)->limit(($page-1)*$limit,$limit)->select();
        if($data){
            foreach ($data as $k=>$v){
                $area = $this->getCity($v['IP']);
                $data[$k]['area'] = $area;
            }
            $res['code'] = 0;
            $res['msg'] = '請求成功';
            $res['count'] = count(Db('line')->where($map)->select());
            $res['data'] = $data;
        }else{
            $res['code'] = 1;
            $res['msg'] = '暂无数据,去添加几条吧!';
    ...
    public function getCity($ip = '')
    {
        ...
        if ($ip == '') {
            $url = "http://api.map.baidu.com/location/ip?ak=E0mNphbGZvF3mMMKSpbTVGkpH6mhTV2O&ip=".$ip."&coor=bd09ll";
...
//            echo $info->getFilename();
    ...
    public function lineDel()
    {
        $id = input('post.id');
        if($id){
            $rsp = Db('line')->where('id',$id)->delete();
            $res['code'] = 0;
            $res['msg'] = '删除成功';
            $res['data'] = $rsp;
        }else{
